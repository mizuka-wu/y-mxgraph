...
// edge target 断裂
const btnCorruptEdgeTarget = document.getElementById(
  "btn-corrupt-edge-target",
) as HTMLButtonElement;
...
btnCorruptEdgeTarget.addEventListener("click", () => {
  withDiagram((cellsMap, cellsOrder, did) => {
    ...
    if (cell && typeof cell.setAttribute === "function") {
      ...
      console.warn(`[debug] diagram ${did}: edge "${edgeId}" 的 target 已改为 "${nonexistentTarget}"`);
      alert(`已将 diagram ${did} 的 edge "${edgeId}" 的 target 改为 "${nonexistentTarget}"\n\n此 target 指向不存在的 cell`);
    }
  });
});
